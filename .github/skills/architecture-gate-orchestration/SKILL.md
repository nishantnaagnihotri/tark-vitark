...
2. **Independence check determines dispatch strategy.** At Gate 4, classify each task pair:
   - **Non-overlapping** (different files, or same file but different lines/blocks with zero merge-conflict risk): orchestrator **may** dispatch tasks in parallel when independence is clear and coordination/rebase overhead is justified; otherwise dispatch sequentially. For parallel dispatch, each agent cuts its branch from `slice/<slice-name>` and opens a PR targeting `slice/<slice-name>`.
