...
14. **Component Showcase is mandatory.** The DS Library page must contain a `Component Showcase` frame that displays every published component in both Light and Dark themes. When a new component is added to the DS Library, a corresponding showcase section (Light + Dark instances) must be appended to the Showcase frame in the same gate run. The Showcase is a human reference artifact — it contains only component instances, never component definitions.
