...
9. When synchronous polling is not possible (orchestrator context — no blocking execute tool):
    - **Orchestrator context** (running in VS Code chat session): use the alarm skill (180 s interval) instead of the poll script. Immediately after `request_copilot_review` returns, arm the alarm and emit the required log entry `[REVIEW REQUESTED] → [POLLING STARTED]` at that moment. In orchestrator context, arming the alarm counts as polling having started for rule 7; alarm wakes are the polling mechanism. On each alarm wake:
      ...
      4. If no review yet from MCP → **do NOT re-request on each wake** — the review was already requested; repeated re-requests flood Copilot's queue. Only re-request again after pushing a new commit. Instead, immediately apply the read-only GitHub REST fallback whenever MCP does not show a review on the current head SHA or the current-head review data is stale/incomplete for the step you are in. Use it to get the latest review directly and bypass MCP cache/lag:
      ```
      gh api repos/<owner>/<repo>/pulls/<number>/reviews --paginate --jq '.[] | select(.user.login == "copilot-pull-request-reviewer[bot]") | {id: .id, commit_id: .commit_id, body: .body[0:120], submitted_at: .submitted_at}' | tail -1
      ```
      Note: `--paginate` is required — without it the response is truncated to the first 30 reviews, silently dropping recent ones. Use `tail -1` to get the chronologically last Copilot review, then compare the returned `commit_id` to the PR head SHA before interpreting the body. If `commit_id` does not match the current head, treat the REST result as stale rather than current-head evidence.
      If the REST response shows a review body indicating zero new comments on the current head SHA (that is, the returned `commit_id` matches the PR head SHA) → treat as `REVIEW_CLEAN` and proceed to the Pre-Completion Self-Check (section 5). If the REST response shows a current-head review with new comments → run PR Review Intake Protocol, address, push, then immediately re-request review and re-arm the alarm so polling restarts atomically after the new review request. If the REST response returns no review on the current head SHA → re-arm the alarm and continue polling while under the 1-hour window. Escalate to PO only if no review arrives within 60 min of `T0` or GitHub workflow state is contradictory to both MCP and REST results.
...
|---|---|---|
| 1 | **Are there any open comments?** Fetch both PR review comments via GitHub MCP `pull_request_read` with `method=get_review_comments` and top-level PR conversation comments via GitHub MCP `issue_read` with `method=get_comments`, then verify there are no unresolved review comments or PR conversation comments that still require a disposition reply from the **PR author**. If MCP thread/comment data is stale, truncated, or incomplete for the current head, an immediate read-only `gh api` fallback is allowed under the GitHub Interaction Policy; no PO approval is required for that read-only review-loop fallback. | Zero unresolved review comments or PR conversation comments awaiting PR-author reply |
