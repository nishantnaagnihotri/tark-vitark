...
10. When a non-MCP polling fallback is needed during a Copilot review wait: if the expected wait is >2 min and no PO decision is required mid-wait, prefer handing off to a VS Code background agent session (see `orchestrator-session-context-lifecycle` skill). If a background handoff is not feasible, run `node scripts/wait_for_copilot_review.js --owner <owner> --repo <repo> --pr <number>` in a background job (e.g., a separate async terminal), redirect its stdout to a temp file (e.g., `/tmp/pr-review-<pr>.json`), and check that file on the next turn. The script prints a single JSON object and exits — there is no live notification stream. Never run the polling script in a blocking/synchronous mode — that holds the entire chat session for the full timeout window with no benefit.
10. When a non-MCP polling fallback is needed during a Copilot review wait: if the expected wait is >2 min and no PO decision is required mid-wait, prefer handing off to a VS Code background agent session (see `orchestrator-session-context-lifecycle` skill). If a background handoff is not feasible, run `node scripts/wait_for_copilot_review.js --owner <owner> --repo <repo> --pr <number>` in a background job (e.g., a separate async terminal), redirect its stdout to a temp file (e.g., `$TMPDIR/pr-review-<pr>.json` on Unix, `%TEMP%\pr-review-<pr>.json` on Windows, or a repo-local temp path like `./.tmp/pr-review-<pr>.json`), and check that file on the next turn. The script prints a single JSON object and exits — there is no live notification stream. Never run the polling script in a blocking/synchronous mode — that holds the entire chat session for the full timeout window with no benefit.
