...
10. When a non-MCP polling fallback is used, always run `node scripts/wait_for_copilot_review.js --owner <owner> --repo <repo> --pr <number>` in **async terminal mode** (`mode=async`) so the chat session is not blocked during the wait. Store the returned terminal ID, then check output on the next turn when the terminal-completion notification arrives. Never run the polling script in sync/blocking mode — that holds the entire chat session for the full timeout window with no benefit.
