...
10. When a non-MCP polling fallback is used and the expected wait is > 2 minutes, prefer handing off to a **background agent session** via VS Code's "Continue in background" option — follow the Background Agent Handoff Protocol in the `orchestrator-session-context-lifecycle` skill. Use `node scripts/wait_for_copilot_review.js --owner <owner> --repo <repo> --pr <number>` only when background agent is unavailable or the wait is expected to be short (< 2 minutes).
