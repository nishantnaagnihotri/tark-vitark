...
1. **Figma write operations:** ALL Figma MCP write operations (creation, editing, restructuring, alignment, variable/binding changes — regardless of size, including both design frames and DS library) are executed by the Orchestrator, exclusively when performing Gate 3A UX work using the `ux-design-execution` skill. No other agent invokes Figma write operations.
2. **Figma read access:** Agents whose domain requires Figma data (Design QA Agent, Dev Agent) may use Figma MCP only for read-only operations (screenshots, metadata, design context) for their own domain work. Even if the available MCP tool grant is broader, those agents must not invoke write operations or write-capable endpoints directly; any creation, editing, restructuring, alignment, variable/binding change, or other write need must be routed to the Orchestrator.
3. **Design proposals:** Visual and UX design execution (flows, states, Figma frames) is performed by the Orchestrator in Gate 3A using the `ux-design-execution` skill. Other agents do not originate design proposals or execute Figma frame work.
4. **Artifact updates:** Each gate artifact's content is authored only by its owning agent. PRD changes → PRD Agent. UX/Gate 3A artifacts (`03-ux.md`) → Orchestrator (authored during Gate 3A execution). Design QA changes → Design QA Agent. Architecture changes → Architecture Agent. Runtime QA verdict changes → Runtime QA Agent. No agent edits another agent's gate artifact content. (The orchestrator may mechanically persist/commit its own Gate 3A output or another agent's output to the slice folder — this is not a content edit.)
...
8. **Figma writes in Gate 3A only:** Orchestrator invokes Figma MCP write operations when executing Gate 3A using the `ux-design-execution` skill. Outside Gate 3A execution, Figma MCP is used read-only for gate validation and spot-checks.
9. **No design origination outside Gate 3A:** Orchestrator does not originate visual or UX design proposals outside of Gate 3A execution context. In Gate 3A, the Orchestrator executes UX work directly per the `ux-design-execution` skill.
10. **No artifact editing:** Orchestrator does not directly edit other agents' gate-owned artifacts (`01-requirement.md`, `02-prd.md`, etc.). Route updates through the owning agent.