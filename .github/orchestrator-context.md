...
5. Design QA Agent: `.github/agents/design-qa.agent.md`
6. Architecture Agent: `.github/agents/architecture.agent.md`
7. Dev Agent: `.github/agents/dev.agent.md`
8. Runtime QA Agent: `.github/agents/runtime-qa.agent.md`
...
41. All Figma MCP write operations (design frames AND DS library) are executed by UX Agent using the `ux-design-execution` skill during Gate 3A. Read-only Figma MCP access is allowed for Design QA Agent, Dev Agent, and Orchestrator for gate validation and spot-checks. Outside Gate 3A execution, no agent invokes Figma write operations. Cross-ref: Known Rule #79.
42. Visual and UX execution (flows, states, Figma frames) is performed by UX Agent in Gate 3A using the `ux-design-execution` skill. Orchestrator routes the async dispatch, validates the returned packet, and manages gate progression.
43. Gate artifact updates route through the owning agent: PRD changes → PRD Agent, Gate 3A/UX artifacts → UX Agent, etc. No agent makes content edits to another agent's gate artifact. Orchestrator may mechanically persist or commit returned output to the slice folder.
...
66. Baseline-lock is mandatory for all continuation slices (any slice adding to or modifying an existing approved screen): UX Agent's first Figma action (Gate 3A, `ux-design-execution` skill) must be MCP `node.clone()` of approved baseline frame(s) — not recreation or approximation. Source node ID and clone node ID must be recorded as provenance in Design Review Access. Any element present in the baseline must come from the clone only; only net-new additions are authored fresh. Violation is a loop-back condition regardless of gate or slice type.
67. M3 Component Library-First: all M3-derived and shared UI components (Card, Button, TextField, SegmentedControl, Chips, NavigationBar, FAB, etc.) must be created and published in the DS library file (`design_system_library_file_key` in `.figma-config.local`) before being used in any slice design frame. Slice files must import components via library link only — no local component definitions allowed for M3-derived components. UX Agent (Gate 3A, `ux-design-execution` skill) must run Component Coverage Check before any frame creation. If a required TV Library component is absent, UX Agent creates it directly in the DS library before proceeding. `UX Readiness: Ready` cannot be claimed if any frame uses a locally-defined component. See `M3 Component Library-First Policy` in `figma-governance-and-fidelity/SKILL.md`.
68. UX Agent is the authoritative Figma execution agent for Gate 3: it produces the `Frame Blueprint` and `DS Component Coverage Declaration` as the authoritative anatomy specification, and then executes Figma frames directly via MCP using the `ux-design-execution` skill in the same Gate 3A pass. DS library gaps surface during Component Coverage Check and are resolved by UX Agent directly in the DS library. Design QA verifies against UX Agent output. There is no separate Figma Agent substep. Cross-ref: Known Rule #79.
...
76. Gate 3 merged execution model (Option C, adopted 2026-04-10; re-routed 2026-04-24): `ux-agent` (via `ux-design-execution` skill) is the single Gate 3A execution agent — it produces both the `03-ux.md` text artifact content and all Figma pixel frames in one pass. Standard Figma file pages: `Design` and `QA Notes` (no `UX Flows` page). Gate 3 substeps: (A) UX+Design single-pass → (B) Design QA with PO approval.
77. Figma Agent eliminated (Protocol 3.17, adopted 2026-04-10): UX Agent owns both slice design frames and DS library management (bootstrap, TV component creation, token/variable updates). Safety constraint: DS library writes must be scoped to a named component task; publish only after verifying no regressions in existing components. There is no separate Figma Agent. Cross-ref: Known Rule #79.
78. Single-screen-first Gate 3A protocol (Protocol 3.18, adopted 2026-04-11): Gate 3A runs in two phases. Phase 1: UX Agent creates only the primary frame (Default/Light/Mobile) and returns it for Product Owner explicit visual approval before any other frames are created. Phase 2 (authorized by PO approval): UX Agent creates all remaining frames. Phase 2 must NOT start until PO has explicitly approved Phase 1. Root cause for this rule: applying variable modes or resizing the outer cloned frame after `duplicate_node` can silently mutate existing elements (topic bar fills, card bubble arrow positions). Phase 1 approval is the mandatory visual gate that catches this class of fidelity failures early. Cross-ref: AGENTS.md Rule 4, `design-gate-orchestration/SKILL.md` Substep A.
...
86. Sync dispatch transparency (adopted 2026-04-23, PO decision): for each sync `runSubagent` handoff, orchestrator must print exactly one sync dispatch banner in chat immediately before the tool call, including role, explicit model, reasoning status (`tool-controlled / not repo-configurable`), and gate/slice context. Cross-ref: `.github/AGENTS.md` Model Routing Policy and `.github/skills/async-agent-dispatch/SKILL.md`.
87. Low-risk implied follow-ons (adopted 2026-04-23, PO decision): within an already approved workflow, orchestrator should autonomously perform the minimal, directly implied, low-risk local follow-on mutations needed to complete the task end-to-end without re-asking for confirmation between obvious local steps such as switching to the target branch, fast-forwarding after a merge, or cleaning up the just-merged local working branch. This implied-follow-on allowance is limited to local-only cleanup; deleting a remote branch, pushing refs, or making any other externally visible mutation still requires explicit confirmation. Confirmation is still required for destructive, irreversible, externally visible, security-sensitive, or scope-expanding actions not already covered by the approved workflow. Cross-ref: `.github/AGENTS.md` Terminal Mutation Override Policy.
88. Gate 3 async-resume verification (adopted 2026-04-24, PO decision): after any async Gate 3A `ux-agent` dispatch, orchestrator may resume Gate 3 only after verifying that `docs/slices/<slice-name>/03-ux.md` exists and contains the final `UX Flow/State Package` plus `Orchestrator Resume Packet`. Async terminal completion alone is not sufficient evidence for gate progression. Cross-ref: `.github/skills/orchestrator-session-context-lifecycle/SKILL.md`, `.github/skills/design-gate-orchestration/SKILL.md`.
89. Gate 3 delta triage (adopted 2026-04-24, PO decision): when UX work, Design QA, or Product Owner discussion during Gate 3 refines the earlier requirement/PRD contract, orchestrator must classify the delta before progression. `Visual-only refinement` stays in Gate 3 artifacts only; `Behavioral refinement within scope` requires explicit Product Owner approval plus AC writeback in `01-requirement.md` and PRD amendment references in `02-prd.md`; `Material scope change` loops back to Gate 2, and to Gate 1 as well if requirement boundaries changed. Unclassified Gate 3 deltas block gate closure. Cross-ref: `.github/skills/design-gate-orchestration/SKILL.md`, `.github/skills/prd-gate-orchestration/SKILL.md`.
...
2. Gate 3 is fully wired through UX+Design single-pass and Design QA. UX Agent owns Gate 3A execution and Figma writes via `ux-design-execution`; orchestrator owns routing, validation, and manual resume.
...
| `create-debate` | ✅ Pass (2026-04-23, Standard) | ✅ Full Pass (2026-04-23) | — | — | — | — |
...
### 2026-04-25 (Non-Dev PR Opening Explicitly Locked)
- Gate status: PR-opening authority clarified further.
- Artifact changes: Tightened shared AGENTS policy, PR review-loop skill, build/merge orchestration, orchestrator contract, and context rule so only Gate 5 dev agents inherit automatic PR-opening authority; all non-dev PR creation requires explicit Product Owner authorization.
- Open questions status: none. This is an ambiguity-removal hardening pass.
...
### Known Rule #79 — UX Agent Restored As Gate 3A Owner (2026-04-24)
- **Decision:** UX Agent restored as the active Gate 3A owner. Orchestrator resumes the gate from returned UX artifacts instead of executing Gate 3A directly.
- **Mechanism:** Orchestrator launches `ux-agent` via async dispatch for Gate 3A. `ux-agent` executes Gate 3A using the `ux-design-execution` skill (`.github/skills/ux-design-execution/SKILL.md`) and must end each pass with a lossless `Orchestrator Resume Packet`.
- **Scope:** Challenge Phase, UX Flow/State Package, Frame Blueprint, DS Component Coverage Declaration, Component Coverage Check, Baseline-Lock, Figma frame execution via MCP, Overlap Check, and `03-ux.md` artifact content are UX-Agent-owned. Orchestrator owns routing, validation, Product Owner review, and mechanical persistence.
- **Files changed:** `AGENTS.md`, `architect-orchestrator.agent.md`, `ux.agent.md`, `design-gate-orchestration/SKILL.md`, `domain-ownership-governance/SKILL.md`, `async-agent-dispatch/SKILL.md`, `gate-handoff-packet/SKILL.md`, `ux-design-execution/SKILL.md`, `scripts/agent-model-routing.ts`, `orchestrator-context.md`.
- **Model:** `ux-agent` default model is `claude-sonnet-4.6`.
- **Rationale:** Restores a dedicated creative UX lane for control selection and interaction-model iteration while preserving deterministic gate resumption through the mandatory `Orchestrator Resume Packet` and explicit async handoff boundary.
