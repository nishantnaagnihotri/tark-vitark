...
82. Global AC numbering (adopted 2026-04-20, PO decision): AC IDs are globally unique across all slices. The Gate 1 owner role (`product-owner`) must determine the latest assigned AC from repository-tracked feature artifacts and seed the next AC from that value at Gate 1 freeze for every new slice. If a slice is expected to add ACs after Gate 1 (e.g., deferred OQs formalized at Gate 3), the gate owner must either (a) reserve those AC IDs at Gate 1 freeze — recording them here with status "reserved, prose deferred to Gate N" — even though their prose is not yet written, or (b) update this rule immediately at the gate where each new AC ID is first assigned, before any subsequent slice may seed IDs. If option (b) is used and Gate 3 assigns new IDs for this slice, update this rule at Gate 3 closure with the new last assigned AC + slice + date. Per-slice feature files remain self-contained, but global uniqueness must be preserved for AC IDs created at any gate.
