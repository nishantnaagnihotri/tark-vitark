...
82. Global AC numbering (adopted 2026-04-20, PO decision): AC IDs are globally unique across all slices. The last AC assigned is **AC-28** (`podium-responsive-layout`, 2026-04-20). The next slice must seed its first AC at **AC-29**. Orchestrator must update this rule (last assigned AC + slice + date) at Gate 1 freeze for every new slice. Per-slice feature files remain self-contained; global uniqueness is enforced at the `01-requirement.md` seed level only.
