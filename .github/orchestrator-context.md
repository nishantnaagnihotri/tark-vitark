...
84. Reasoning-effort policy (adopted 2026-04-23, PO decision): all repo-controlled Copilot SDK agent sessions must use the highest supported `reasoningEffort` for the selected model. `scripts/run-agent.ts` and `scripts/mcp-dev-orchestrator.ts` enforce this centrally by inspecting `listModels()` metadata and selecting the strongest supported level, with `high` as the fallback when model metadata is unavailable. Sync `runSubagent` handoffs currently expose explicit model selection but no repo-controlled reasoning-effort parameter; this is tracked as a tool limitation rather than a policy exception. Cross-ref: `.github/AGENTS.md` Model Routing Policy.
