...
15. For UI-impacting issues, after the PR is open or updated on the latest head, invoke `test` yourself before final handback. In that handoff, explicitly pass `Validation Scope: Issue-Level Gate 5.5`, or `Validation Scope: Slice-Level Integrated` when the validation is slice-level, alongside the journey map, route list, expected states, setup notes, setup/test data, known-risk notes, and any applicable Figma node IDs. Treat the returned runtime verdict as authoritative for Gate 5.5 evidence; if it fails, loop back to fixes and rerun `test` validation before claiming completion. Use `runtime-qa` only as a compatibility fallback during the #229-#234 transition window.
16. For UI-impacting issues, PR-body evidence images must use a non-UI agent-controlled URL path in the PR description. Default to configured external evidence storage with absolute HTTPS URLs keyed by PR number and exact pushed 40-character head SHA. If configured external evidence storage is unavailable, fall back to GitHub-hosted attachment URLs from a non-UI upload path or repository-hosted evidence files committed under a PR-scoped folder on the task branch and referenced inline from URLs pinned to the exact pushed 40-character head SHA. Use browser/UI upload only when Product Owner explicitly requires GitHub-hosted attachment URLs for the current PR and no non-UI upload capability exists. Never guess, shorten, or hand-copy the evidence SHA; read the actual pushed head SHA and validate that the images resolve before claiming completion.
...
12. For UI-impacting issues, perform in-agent visual validation: start the development server locally, navigate to the relevant route/screen, and capture a browser screenshot for each required viewport and theme (Light and Dark) using Chrome DevTools MCP. Retrieve the corresponding Figma frame screenshot via Figma MCP for the same node IDs. Produce a side-by-side parity assessment (exact match / intentional deviation / regression) for each viewport+theme pair. Store the evidence images for the current PR under a PR-scoped folder such as `screenshots/runtime-qa/pr-<pr-number>/`. After the evidence set is ready and the exact pushed 40-character head SHA is known, prefer configured external evidence storage and upload it via `npm run upload:evidence -- --prefix pr-<pr-number>/<head-sha> <evidence-path>`. Use the returned absolute HTTPS URLs in the PR body and validate that they resolve before final handback. If configured external evidence storage is unavailable, fall back to GitHub-hosted attachment URLs from a non-UI upload path or to repository-hosted file URLs built from the exact pushed head SHA. Use browser/UI upload only when Product Owner explicitly requires GitHub-hosted attachment URLs and no non-UI upload capability exists. If a regression is found, fix before opening the PR.
...
15. For UI-impacting issues, after the PR head is pushed and before final handback, invoke `test` on explicit model `gpt-5.4` using the current PR link and latest head context. If verdict is `Fail`, `Blocked`, or `AC-DELTA`, fix or escalate before claiming `Build Readiness: Ready`. Use `runtime-qa` only as a compatibility fallback during the #229-#234 transition window.
...
14. For UI-impacting issues, browser screenshot evidence (at all required viewports and themes) paired with Figma reference screenshots is rendered inline in the PR body via either absolute HTTPS URLs from configured external evidence storage, GitHub-hosted attachment URLs from a non-UI upload path, or repository-hosted file URLs pinned to the exact current-head 40-character SHA; any intentional deviation is explicitly noted with justification.
15. PR description accuracy: before opening the PR, run `git diff --stat origin/<base-branch>` and verify every file in the diff is listed in the PR body's `Files Changed` section, and every file listed in `Files Changed` appears in the diff. No fabricated entries, no silent omissions. External-storage evidence URLs and GitHub-hosted attachment URLs are not branch files and must not be listed in `Files Changed`. When repository-hosted evidence files are used, they are branch files and must be listed in `Files Changed`.
