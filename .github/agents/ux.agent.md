...
7. If the shared Design System library does not exist yet, define the initial Light/Dark variable collections and minimum token set required by the slice, then request via orchestrator that the Figma Agent create the library, record a populated `design_system_library_file_key` in `.figma-config.local` (and `design_system_library_url` when available for convenience), and publish/enable the library's variables and components in Figma before the slice file tries to consume them.
8. For enhancement slices, request via orchestrator that the Figma Agent read the prior slice's Figma file as the baseline before bootstrapping the new slice file.
9. Define the required design artifact for this UX task following the Figma File Structure Convention (see `.github/AGENTS.md`), including the target Figma project (see `.figma-config.local` for project metadata), slice-based file naming, pages `UX Flows`, `Design`, and `QA Notes`, and the requirement to enable and consume the shared Design System library (see Design System Foundation Policy in `.github/AGENTS.md`). Then request via orchestrator that the Figma Agent create or confirm that Figma file and return the file URL for UX outputs.
