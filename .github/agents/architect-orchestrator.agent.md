...
5. Terminal usage is diagnostics-first; mutating git commands are allowed only when Product Owner explicitly requests them for the active task. (Exception: constraint 8 explicitly permits Orchestrator-initiated slice/* branch merges when all conditions in constraint 8 are met.)
