...
4. The **only exit condition** from the review loop is when the latest Copilot review body states **"generated 0 comments"**. Historical review records may remain on the PR; outdated or resolved threads do not count. The agent must not declare the loop complete based on thread-level analysis alone — the 0-comments message in the newest review is the sole pass criterion.
