...
## Role-Based SDLC Migration Decision Freeze (Issue #228)

1. Visible workflow role contract is frozen to exactly five names: `product-owner`, `ux-ui`, `architect`, `dev`, `test`.
...
5. Gate 3B default path uses sync `runSubagent` from `ux-agent` to `design-qa-agent` with an explicit role-default model (currently `qwen/qwen3.6-35b-a3b` unless deliberately overridden). Orchestrator consumes the returned critique and persists the latest Gate 3B pass to `04-design-qa.md` before Product Owner review.
...
4. If a required GitHub action or required current GitHub data is not available through the GitHub MCP server, the agent must explicitly state the MCP gap before using any fallback path.
5. For PR-body evidence images, agents must prefer a non-UI path. Accepted non-UI evidence URL forms are: (a) absolute HTTPS URLs from configured agent-controlled external evidence storage, keyed by PR number and exact pushed 40-character PR head SHA, (b) GitHub-hosted attachment URLs when a non-UI upload capability exists, or (c) repository-hosted file URLs pinned to the exact pushed 40-character PR head SHA.
6. When external evidence storage is used for PR evidence, agents must upload only the current PR's evidence images under a PR-scoped and current-head-scoped prefix, generate the inline URLs only after the upload completes, use absolute HTTPS URLs, and validate that the images resolve before claiming completion. External-storage evidence is not a branch diff artifact and must not be listed in a PR body's `Files Changed` section.
7. When repository-hosted file URLs are used for PR evidence, agents must commit only the current PR's evidence images in a PR-scoped folder (for example `screenshots/runtime-qa/pr-<pr-number>/`), generate the inline URLs only after the evidence commit is pushed, use the exact pushed 40-character head SHA in every URL, and validate that the images resolve before claiming completion.
8. If Product Owner explicitly requires GitHub-hosted attachment URLs for the current PR and the GitHub MCP server still lacks non-UI upload capability, agents may use browser/UI tooling as a sanctioned fallback to upload the evidence images to GitHub and capture the resulting attachment URLs without additional Product Owner approval. This exception is limited to evidence-image upload and URL capture for the current PR package; other GitHub mutations remain MCP-first unless another explicit rule applies.
9. Aside from the sanctioned PR-evidence attachment fallback above, Product Owner approval is still required before any mutating or externally visible fallback action. Exception: during the Copilot PR review loop, agents may immediately use read-only GitHub REST fallbacks without Product Owner approval to read reviews, review threads/comments, PR conversation comments, and check/workflow state when MCP data is missing, stale, truncated, or otherwise insufficient to determine the current-head review status.
