...
const AUTH_SCHEME = (process.env.PENPOT_AUTH_SCHEME || "Token").trim();
...
function normalizePayloadKeys(value) {
    ...
    if (value && typeof value === "object") {
        ...
        for (const [key, inner] of Object.entries(value)) {
            out[toKebabKey(key)] = normalizePayloadKeys(inner);
        }
        return out;
    }
...
async function requestPenpot({ method = "GET", path = "/", query, body, headers = {} }) {
    ...
    if (TOKEN) {
        ...
        if (/^(Bearer|Token)\s+/i.test(tokenValue)) {
            requestHeaders.Authorization = tokenValue;
        } else {
            requestHeaders.Authorization = `${AUTH_SCHEME} ${tokenValue}`;
        }
    }
    ...
    if (body !== undefined) {
        ...
        requestBody = JSON.stringify(normalizePayloadKeys(body));
    }
