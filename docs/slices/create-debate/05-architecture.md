...
| Schema validation | Use a small manual runtime guard in the storage adapter, not zod in client code. | This slice only needs a narrow storage-boundary check, and a manual guard keeps the adapter contract explicit and simple without introducing extra client-side schema runtime. |
